...
**投稿はサーバーを通さない。** 投稿テキストは結果画面のメモリ上の値からクライアントで組み立てるため、サーバーAPIもDBも関与しない。ランキングへの登録（`docs/04-api.md`）とは独立していて、登録しなくても投稿できる。
...
- テーマモード: `HENGEで「<テーマ名>」の<短文|単語>を打った。`
- 最適化モード: `HENGEで「<最適化の文字>」の最適化練習を打った。`（テーマ名が1〜4文字のかなのため、文として成立させる。**最適化練習に単語モードは無い**ので形式を書き足さない）
- **形式は短文も単語も明記する。** 同じテーマ名で中身も問題数も違うため、書かないとどちらの記録か分からない。単語のときだけ書く形にすると「書いていない＝短文」という読み方を求めることになり、記録として弱い
- **問題数は書かない。** 形式ごとに固定（短文15／単語20）なので、形式が書いてあれば足りる。結果画面と同じ判断
- スコア: `etypingScore()` の値。打鍵/秒は小数1桁、正確率は切り捨て整数（結果画面の表記と同じ。`packages/shared/src/score.ts`）
...
- `Result` には `kind` が渡っていないため、`PlayScreen`（`kind` を知っている）から共有URLを渡す。**形式（`form`）も同じ経路で渡す**（結果画面の見出しの問題数と、投稿の文面の両方で使う）
...
- **プレイごとのスコア履歴**（スコープ外）。投稿後に結果を参照する術は無い（ランキングに登録したベストだけが残る）
