...
| POST | `/api/sessions/start` | 不要（匿名可） | プレイ開始。1プレイ分のお題を返す |
| POST | `/api/themes` | 必須 | 新規作成（初回10問を同期生成） |
...
// form は "sentence"（既定）/ "word" / "long"。省略すると短文になる
...
- **1プレイの問題数は形式で変わる**（短文10／単語20／長文1本）。プールもオフセットも形式ごとに別（`docs/03-data-model.md`）
- 問題はシャッフルして返す
- **返却した時点でオフセット消費が確定する**（中断しても巻き戻さない）
- ログイン時のみ、条件を満たせば`ctx.waitUntil()`でバックグラウンド生成をキックする

**`form` は Zod で `sentence` / `word` / `long` だけを許し、省略時は `sentence` に倒す。** 形式を持たない古いURL・古いクライアントがそのまま短文で動く。知らない値を通すと、存在しないプールをHonoに引かせることになる。
...
{ "theme": { "id": "...", "kind": "theme", "name": "忍びの心得",
             "promptCounts": { "sentence": 17, "word": 0, "long": 0 } },
  "created": true }   // falseなら既存テーマの再利用（生成は走っていない）
```

**新規作成で作るのは短文だけ。** 単語・長文を同じリクエストで作れない（不変条件4）。2形式ぶんを同期で回すと、待たせる時間と失敗確率がそのまま倍になる（1形式でも最大2ラウンドある）。**単語・長文のプールは `POST /api/prompts/regenerate` で別リクエストとして作る。**
...
// レスポンス
{ "theme": { ..., "promptCounts": { "sentence": 30, "word": 22, "long": 0 } },
  "added": 22, "neuronsUsed": 13.0, "neuronsRemaining": 487 }
```

**単語と長文は、目標に届かなくても取れた分を保存する。** 短文は目標（1プレイ分）未達なら `GENERATION_FAILED` を返して1件も保存しないが、単語は1回で作れるのが最大80件（40件×2ラウンド）、長文は最大10本（5本×2ラウンド）しかなく、届かないことが起こりうる。同じ扱いにすると**有効な20語と消費したニューロンを捨てて、何度も押させる**ことになる。追加先は既存テーマのプールなので、途中まで積むこと自体に害はない（「お題ゼロのテーマを作らない」新規作成とは事情が違う）。**1件も作れなかったときだけ失敗として返す。**
...
**最適化練習（`kind: 'constraint'`）に `sentence` 以外の形式を指定すると `VALIDATION_ERROR`。** 単語・長文はテーマだけ（`docs/05-generation.md`）。
...
// 生成時と同じ検査（文字種・漢字・その形式の打鍵数の範囲・長文は文の区切り・「含む」文字）を通してから
...
| GET | `/themes/:id` | 単体取得 |
| POST | `/sessions/start` | 1プレイ分の取得＋オフセット更新＋補充要否判定 |
| POST | `/themes` | 作成＋初回10問の同期生成 |
...
| `GENERATION_FAILED` | 422 | リトライ上限でも10問未満 | テーマ名の変更を促す |
...
枠を分けているのは、遊んでいるだけでテーマ作成の予算が減るのを避けるため。**補充が要るかどうかは在庫数を持つHono側にしか分からない**ので、Next.js は補充の不要なプレイでも1消費する。そのため `REFILL_RATE_LIMIT` の上限は補充の頻度ではなく**プレイの頻度**で見積もっている。1プレイ10問なので人間が1分に10回セッションを開始することはなく、実質スクリプト連打にだけ効く。これが無いと、在庫の少ないテーマを並べてクォータ残（最大50件）の補充を一斉に走らせられる。
