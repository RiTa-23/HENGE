...
/**
 * テーマモードと「含む文字」モードの**URL上の区別**をここだけに閉じ込める。
 *
 * 一意制約が `(kind, normalized_name)` なので、テーマ名「ざ」と含む文字「ざ」は
...
/** 一覧・詳細ページへのリンク。テーマは `/themes/[name]`、含む文字は `/practice/[char]` */
...
/**
 * 画面に出す呼び名。「テーマ」という語は含む文字モードでは通じない
 * （利用者が指定したのはテーマ名ではなく1文字の音のため）。
 */
export function kindLabel(kind: ThemeKind): string {
  return kind === "constraint" ? "この文字" : "このテーマ";
}
