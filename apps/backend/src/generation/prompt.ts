...
function rules(count: number): string[] {
  return [
    ...
    "- 最初の2音が同じ文を3本以上作らない。文の切り出し方（主語・修飾語・場所）を毎回変える",
    "- テーマを代表する語を、複数の文の主題にしない。文ごとに違う側面（定義・種類・生息地・使い方・歴史など）を扱う",
...
/**
 * テーマモード固有の指示。**テーマ名の使いすぎを止める。**
 *
 * テーマ「魚」で生成すると「魚は海と川にいる」「魚は泳ぎます」のように
 * テーマ名が文頭に並んだ。モデルはテーマを1語に要約して、全部の文を
 * その語で組み立てる。数えられる形で指示する（docs/05-generation.md）。
 */
function themeRules(name: string): string[] {
  return [
    `- テーマの名前「${name}」そのものを、文中に使うのは最大2文まで。他の文は名前を出さずに内容を説明する`,
  ];
}
