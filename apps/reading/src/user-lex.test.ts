...
describeWithDict("ユーザー辞書で誤読が直る", () => {
  ...
  test.each([
    ...
    // レビューで発見したベースライン誤読（語幹系の追加で修正）
    ["聞きなさいと言った", "ききなさいといった"],
    ["ドアが閉まった", "どあがしまった"],
    ["窓が閉まっていた", "まどがしまっていた"],
    ["泣くなと言った", "なくなといった"],
    ["泣く子も黙る", "なくこもだまる"],
    // 文脈依存別読み: 中(ナカ) は「の中+助詞」で文脈を束縛、
    // 入っ は ハイッ を強制勝ちにして いっ系複合語は語幹エントリで保護
    ["甘さが口の中に広がる", "あまさがくちのなかにひろがる"],
    ["賑やかな中で楽しむ", "にぎやかななかでたのしむ"],
    ["雨の中を歩く", "あめのなかをあるく"],
    ["森の中まで歩く", "もりのなかまであるく"],
    ["トッピングにはみかんが入っている", "とっぴんぐにはみかんがはいっている"],
    ["ドアに入った", "どあにはいった"],
    ["箱に入ってみる", "はこにはいってみる"],
    ["気に入る色", "きにいるいろ"],
    ["差し入る季節", "さしいるきせつ"],
  ...
  test.each([
    ...
    // 「な」は誤読側(ナン)のエントリと同じ lid/rid(303,5296) を借りて
    // コストを少しだけ下げた(7500 < 7683)差し替え型。単純にコストを
    // 下げたナ entry(-20000等)だと 大きな→だいきな・行きなさい→いくきなさい
    // のような分割回帰を誘発する
    ["大きな声で呼ぶ", "おおきなこえでよぶ"],
    ["行きなさいと言った", "いきなさいといった"],
    // 聞き は 3000。-20000 だと 聞き込み→ききごみ と連濁複合語を分解する
    ["聞き込みをする", "ききこみをする"],
    ["聞き取り調査", "ききとりちょうさ"],
    ["閉店セール", "へいてんせーる"],
    ["泣き虫な弟", "なきむしなおとうと"],
    // の中X/な中X は「中の後に助詞が要る」ので中で始まる複合語を分解しない
    ["彼の中国旅行", "かれのちゅうごくりょこう"],
    ["世界の中華料理", "せかいのちゅうかりょうり"],
    ["試合中に怪我をした", "しあいじゅうにけがをした"],
    ["授業中に寝る", "じゅぎょうじゅうにねる"],
    ["中央駅に着く", "ちゅうおうえきにつく"],
    // 入っ は ハイッ 単独で強制勝ち。いっ系複合語は語幹エントリで保護
    ["気に入った色", "きにいったいろ"],
    ["手に入った本", "てにいったほん"],
    ["立ち入った場所", "たちいったばしょ"],
    ["差し入れた", "さしいれた"],
