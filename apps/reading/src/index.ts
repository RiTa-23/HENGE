 ...
 *
 * 辞書（IPADIC、46MB）を Wasm の中に持つので、Hono Worker とは分けてある。
 * 同居させると、プレイやランキングを捌くアイソレートにも辞書のメモリと起動が乗る。
 */
...
async function loadReader(env: Env): Promise<Reader> {
  ...
  const response = await env.ASSETS.fetch("https://assets/ipadic.dic.zst");
  if (!response.ok) throw new Error(`辞書が読めない: ${response.status}`);
  // zstd 圧縮のまま渡し、Wasm の中でストリーム展開する。
  // JS 側で展開すると 46MB のバッファが JS と Wasm の両方にできて 128MB を超える
