...
{ "score": 287,      // サーバーで計算したスコア（結果画面と同じ関数）
  "best": true,      // ベストを更新したか。false なら何も書いていない
  "rank": 3 }        // 保存されている記録の順位。100位以内でなければ null
```
...
- 記録は自己申告で改ざんは防げない（MVPでは対策しない）。Zod で弾くのは**打っていないと分かる値**だけ（`playStatsRejection`: 1プレイの打鍵数の範囲・時間が0以下または1時間超・30打鍵/秒超）
