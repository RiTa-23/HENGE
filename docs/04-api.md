...
- **1プレイの問題数は形式で変わる**（短文15／単語20／長文1本）。プールもオフセットも形式ごとに別（`docs/03-data-model.md`）
- 問題はシャッフルして返す
...
**新規作成で作るのは短文だけ。** 単語・長文を同じリクエストで作れない（不変条件4）。1回の実行で読み仮名の取得に使える外部サブリクエストは50回で、2形式ぶんだと最大80回になり静かに失敗する。`waitUntil` も同じ実行の枠を食うので回避にならない。**単語・長文のプールは `POST /api/prompts/regenerate` で別リクエストとして作る。**
...
**単語と長文は、目標に届かなくても取れた分を保存する。** 短文は目標（1プレイ分）未達なら `GENERATION_FAILED` を返して1件も保存しないが、単語は1回で作れるのが最大40件（20件×2ラウンド）、長文は最大10本（5本×2ラウンド）しかなく、届かないことが起こりうる。同じ扱いにすると**有効な20語と消費したニューロンを捨てて、何度も押させる**ことになる。追加先は既存テーマのプールなので、途中まで積むこと自体に害はない（「お題ゼロのテーマを作らない」新規作成とは事情が違う）。**1件も作れなかったときだけ失敗として返す。**
