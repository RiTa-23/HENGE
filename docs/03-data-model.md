...
このインデックス1本で、配信・管理一覧のページネーション（`ORDER BY sequence_number LIMIT ? OFFSET ?`）と**次の採番**（`SELECT MAX(sequence_number)`）を賄える。**在庫数は `COUNT(*)` で数える。** 管理画面からお題を1件消すと連番に穴が空くため、最大値で数えると在庫を多く見積もりすぎる。配信も連番の「値」で範囲指定せず、行数で位置を数える（`OFFSET`）。そうしないと穴を含む1プレイ分のブロックが足りなくなり、1件の削除でテーマが遊べなくなる。

**テーマ行はお題10問と同じバッチで挿入する。** テーマ行を先に作ると、生成失敗時にお題ゼロのテーマが公開一覧に残り、クリックしても何も遊べない状態になる。
...
| `play_count` | INTEGER | その形式の1プレイ分（短文10／単語20）の倍数。次に配信する範囲のオフセット |
...
お題1件の挿入で**10個**使う（`form` を足して1つ増えた）ため、**11件以上を1文で挿入すると `too many SQL variables` で落ちる**。10件でもちょうど上限100個に張り付くので、`apps/backend/src/db/prompts.ts` では**9件ずつ**に分けている。生成は1ラウンド最大40件（単語）なので分割は必須。テーマ行とお題は分割してもバッチの中に収める。**列を足すときはこの数を見直すこと。**
...
|---|---|---|---|---|
| `sentence` | 10問 | 20 | 10〜35 | 20 |
| `word` | 20問 | **40** | 4〜20 | 40 |
| `long` | **1本** | **3** | **250〜450** | **5** |

**在庫目標は、1回の補充で作れる上限を超えない値にする。** 単語の在庫目標40に対し
補充が1回で作れるのは最大80件（40件×2ラウンド）なので、在庫0からでも埋まる。
上限を超える目標を掲げると、在庫0からは採用率100%でも届かず、目標未達で
`'difficult'` を立てる条件に当たるため、**普通に作れているテーマに「生成困難」の
印が付き、以後の補充が止まる**。
...
短文1プレイ（約220打）と同じ程度の手応え。在庫目標3本は、1回の補充で作れる上限（5本×2ラウンド＝10本）に
