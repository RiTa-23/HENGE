...
- **1プレイの問題数は形式で変わる**（短文15／単語30）。プールもオフセットも形式ごとに別（`docs/03-data-model.md`）
- 問題はシャッフルして返す
...
**単語だけ、目標に届かなくても取れた分を保存する。** 短文は目標（1プレイ分）未達なら `GENERATION_FAILED` を返して1件も保存しないが、単語は目標30に対し1回で作れるのが最大40件（20件×2ラウンド）しかなく、少し届かないことが普通に起きる。同じ扱いにすると**有効な20語と消費したニューロンを捨てて、何度も押させる**ことになる。追加先は既存テーマのプールなので、途中まで積むこと自体に害はない（「お題ゼロのテーマを作らない」新規作成とは事情が違う）。**1件も作れなかったときだけ失敗として返す。**
