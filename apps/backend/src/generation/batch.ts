 ...
 * 短文の1ラウンドあたりのリクエスト件数。
 *
 * **各形式の依頼数 ≤ `MAX_TEXTS`（読み Worker の1回あたりの上限40）を必ず満たすこと。**
 ...
 * 単語の1ラウンドの件数。
 *
 * 1語あたりの出力が短いので短文より多く頼める。在庫目標40件に対し、
 * 40件×2ラウンド＝最大80件で、却下や重複を差し引いても埋まる。
 */
export const N_REQUEST_WORD = 40;
export const MAX_ROUNDS = 2;

/**
 * 長文の1ラウンドの件数。**1本が長いので少なく頼む。**
 *
 * 1本≒短文1プレイぶんの出力で、20本頼むと応答が長すぎる（時間も `max_tokens` も）。
...
export function requestCount(form: PromptForm): number {
  if (form === "word") return N_REQUEST_WORD;
  if (form === "long") return N_REQUEST_LONG;
  return N_REQUEST;
}
 ...
 *
 * 短文は目標（1プレイ分＝10問）未達なら1件も保存せず失敗にするが、単語と長文は
 * **目標と1回で作れる上限が近い**（単語: 目標40に対し最大80件、長文: 目標3に対し
 ...
 * 「ぴ」で40件中18件、「ぬ」で11件がここで落ち、10問に届かなかった（実測）。
