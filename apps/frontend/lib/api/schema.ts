 ...
 */
/**
 * お題本文の上限（表記の文字数）。**画面の `maxLength` と検証で同じ値を使う。**
 *
 * 長文は表記で150〜250文字になる（打鍵450 ≒ かな220文字）。100のままだと
 * 管理画面で長文を編集した時点で必ず弾かれる（`<input maxLength>` の側は、
 * 超えている本文への**追加入力だけを黙って捨てる**ので、原因が見えない）。
 * 打鍵数の本当の上限は Hono 側が読み仮名から検査するので、ここは形式に依らず
 * 十分に大きい枠でよい。
 */
export const PROMPT_TEXT_MAX_LENGTH = 400;

export const promptTextSchema = z.object({
  text: z.string().trim().min(1).max(PROMPT_TEXT_MAX_LENGTH),
});
